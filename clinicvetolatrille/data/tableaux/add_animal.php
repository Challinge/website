...
if(isset($_SESSION['id']) AND isset($_SESSION['logged']) AND $lifeValidation){
	...
	if (trim($resultat['validiteutilisateur']) == 'valide' AND trim($resultat['connectutilisateur']) == 'connected' AND $lifeValidation){
		...
		if (trim($result['nomanimal']) != '' AND trim($result['race']) !='' AND trim($result['robe']) !='' AND
				trim($result['datedenaissance']) !='' AND trim($result['sexe']) >= '' AND (int) trim($result['poids']) > 0
				AND (int) trim($result['idproprietaire']) > 0 ){
			...
			$req = $bdd->prepare('INSERT INTO animal(idproprietaire, nomanimal, race, robe, datedenaissance, sexe, poids, idutilisateur)
					VALUES(:idproprietaire, :nomanimal, :race, :robe, :datedenaissance, :sexe, :poids, :idutilisateur)');
			$req->execute(array(
					'idproprietaire' => $animal -> idproprietaire,
			...
			print json_encode(array("success" => true , "msgjson" => "Informations sur l'animal ajout&eacute;es avec succ&egrave;s."));
		}
		else{
			print json_encode(array("success" => false , "msgjson" => "Informations sur l'animal non ajout&eacute;es. Veuillez resaisir correctement ces informations."));
