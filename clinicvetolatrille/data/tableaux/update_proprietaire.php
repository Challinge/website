...
if(isset($_SESSION['id']) AND isset($_SESSION['logged']) AND $lifeValidation){
	...
	if (trim($resultat['validiteutilisateur']) == 'valide' AND trim($resultat['connectutilisateur']) == 'connected' AND $lifeValidation){
		...
		if (trim($result['nomproprietaire']) != '' AND trim($result['profession']) !='' AND trim($result['adressegeograph']) !='' AND
				...
				trim($result['telportable']) > 0 AND trim($result['idproprietaire']) > 0){
			...
			//Requête UPDATE sur la BDD.
			$req = $bdd->prepare('UPDATE proprietaire SET nomproprietaire = :nomproprietaire,
					profession = :profession, adressegeograph = :adressegeograph, adressepostale = :adressepostale,
					telbureau = :telbureau, teldomicile = :teldomicile, telportable = :telportable
			...
			print json_encode(array("success" => true , "msgjson" => "Informations concernant le propri&eacute;taire modifi&eacute;es avec succ&egrave;s."));
			$_SESSION['timeout'] = time();
		}
		else{
			print json_encode(array("success" => false , "msgjson" => "Informations concernant le propri&eacute;taire non modifi&eacute;es. Veuillez resaisir correctement ces informations."));
			$_SESSION['timeout'] = time();
		}
	}else{
		//Récupération des données envoyées par la requête.
