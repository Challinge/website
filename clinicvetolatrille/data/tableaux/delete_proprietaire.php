...
if(isset($_SESSION['id']) AND isset($_SESSION['logged']) AND $lifeValidation){
	...
	if (trim($resultat['validiteutilisateur']) == 'valide' AND trim($resultat['connectutilisateur']) == 'connected' AND $lifeValidation){
		...
		if (trim($result['nomproprietaire']) != '' AND trim($result['profession']) !='' AND trim($result['adressegeograph']) !='' AND
			...
			$req = $bdd->prepare('DELETE FROM proprietaire WHERE idproprietaire = :idproprietaire');
			$req->execute(array(
					'idproprietaire' => $proprietaire -> idproprietaire
			));

			//Envoie du r�sultat au client.
			print json_encode(array("success" => true , "msgjson" => "Informations concernant le propri&eacute;taire supprim&eacute;es avec succ&egrave;s."));
			$_SESSION['timeout'] = time();
		}
		else{
			print json_encode(array("success" => false , "msgjson" => "Informations concernant le propri&eacute;taire non supprim&eacute;es. Veuillez ne saisir aucune information."));
			$_SESSION['timeout'] = time();
		}
	}else{
		//Récupération des données envoyées par la requête.
