...
if(isset($_SESSION['id']) AND isset($_SESSION['logged']) AND $lifeValidation){
	...
	if (trim($resultat['validiteutilisateur']) == 'valide' AND trim($resultat['connectutilisateur']) == 'connected' AND $lifeValidation){
		...
		if (trim($result['nomanimal']) != '' AND trim($result['race']) !='' AND trim($result['robe']) !='' AND
			...
			//Requête UPDATE sur la BDD.
			$req = $bdd->prepare('UPDATE animal SET nomanimal = :nomanimal,
					race = :race, robe = :robe, datedenaissance = :datedenaissance,
					sexe = :sexe, poids = :poids  WHERE idanimal = :idanimal');
			$req->execute(array(
			...
			print json_encode(array("success" => true , "msgjson" => "Informations concernant l'animal modifi&eacute;es avec succ&egrave;s."));
			$_SESSION['timeout'] = time();
		}
		else{
			print json_encode(array("success" => false , "msgjson" => "Informations concernant l'animal non modifi&eacute;es. Veuillez resaisir correctement ces informations."));
			$_SESSION['timeout'] = time();
		}
	}else{
		//Récupération des données envoyées par la requête.
