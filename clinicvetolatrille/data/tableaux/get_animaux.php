...
if(isset($_SESSION['id']) AND isset($_SESSION['logged']) AND $lifeValidation){
	...
	if (trim($resultat['validiteutilisateur']) == 'valide' AND trim($resultat['connectutilisateur']) == 'connected' AND $lifeValidation){
		...
		$rep = $bdd -> prepare('SELECT * FROM animal ORDER BY idanimal DESC LIMIT :start, :limit');
		...
		while ($donnees = $rep -> fetch()) {
			...
			//classe Animal.
			$animal = new Animal($donnees['idanimal'], $donnees['nomanimal'], $donnees['race'],
		...
		$rep = $bdd -> query('SELECT COUNT(*) as nombretotalanimaux FROM animal');
		$donnees = $rep -> fetch();
		$rep -> closeCursor();
