...
if(isset($_SESSION['id']) AND isset($_SESSION['logged']) AND $lifeValidation){
	...
	if (trim($resultat['validiteutilisateur']) == 'valide' AND trim($resultat['connectutilisateur']) == 'connected' AND $lifeValidation){
		...
		if (trim($result['dates']) != '' AND trim($result['symptomes']) !='' AND trim($result['diagnostic']) !='' AND
				trim($result['traitements']) !='' AND trim($result['typesoin']) !='' AND (int)trim($result['idsoins']) > 0
				AND (int)trim($result['idanimal']) > 0){
			...
			//Requête UPDATE sur la BDD.
			$req = $bdd->prepare('UPDATE soins SET dates = :dates,
					symptomes = :symptomes, diagnostic = :diagnostic, typesoin = :typesoin, traitements = :traitements,
			...
			print json_encode(array("success" => true , "msgjson" => "Informations concernant le soin modifi&eacute;es avec succ&egrave;s."));
			$_SESSION['timeout'] = time();
		}
		else{
			print json_encode(array("success" => false , "msgjson" => "Informations concernant le soin non modifi&eacute;es. Veuillez resaisir correctement ces informations."));
			$_SESSION['timeout'] = time();
		}
	}else{
		//Récupération des données envoyées par la requête.
